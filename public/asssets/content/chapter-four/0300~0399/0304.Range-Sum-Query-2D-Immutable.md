...
![https://leetcode.com/static/images/courses/range_sum_query_2d.png](/images/chuong-4/sum-grid.jpg)
...
- 于是得到递推的关系式：`f(i, j) = f(i-1, j) + f(i, j-1) - f(i-1, j-1) + matrix[i][j]`，写代码为了方便，新建一个 `m+1 * n+1` 的矩阵，这样就不需要对 `row = 0` 和 `col = 0` 做单独处理了。上述推导公式如果画成图也很好理解：
    ...
    ![https://img.halfrost.com/Leetcode/leetcode_304.png](/images/chuong-4/leetcode_304.png)
